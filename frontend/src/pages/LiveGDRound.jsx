...
function LiveGDRound() {
  ...
  const registerSocketListeners = () => {
    ...
    socketRef.current.on("live-gd-join-request", (request) => {
      ...
      speakText(
        `${request.name || "A participant"} is waiting to join.`,
        "Moderator"
      )
    })
  ...
  const createRoom = async () => {
    ...
    try {
      ...
      if (!newRoundId || !code) {
        throw new Error(
          "Meeting created but room ID/code missing. Check backend response."
        )
      }
      ...
      speakOpeningMessages(room?.messages || [])
    } catch (err) {
